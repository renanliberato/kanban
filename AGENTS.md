...
- Add board workflow columns in `src/core/board-columns.ts`. The shared definitions drive the API schema, persisted-board normalization, CLI column parsing, project counts, board/detail UI, drag rules, settings UI, and stage automation. For an automated stage, set `automation.promptTemplateDefault`, `failurePromptTemplateDefault`, `passSignal`, and `failSignal`; prompts are persisted in the generic `stagePromptTemplates` and `stageFailurePromptTemplates` maps.
